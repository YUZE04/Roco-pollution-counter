...
def _install_paddlex_official_models_stub():
    """使用本地 OCR 模型时，避免 PaddleX 导入在线模型下载依赖链。"""
    module_name = "paddlex.inference.utils.official_models"
    if module_name in sys.modules:
        return False

    class _OfflineOfficialModels:
        def __getitem__(self, model_name):
            raise RuntimeError(
                f"当前打包版未包含在线模型下载依赖，无法自动拉取官方模型：{model_name}"
            )
...
_install_paddlex_official_models_stub()
...
def _add_paddle_dll_search_paths():
    """补充 Paddle 运行时 DLL 搜索路径，避免打包后找不到 mklml.dll。"""
    try:
...
def _patch_paddlex_ocr_core_dependency_check():
    """打包环境下让 PaddleX 用模块可导入来判断 OCR core 依赖。"""
    try:
        import importlib.util
        import cv2
    ...
    def _patched_is_dep_available(dep, /, check_version=False):
        result = original_is_dep_available(dep, check_version=check_version)
        if result:
            return True
    ...
    try:
        original_is_extra_available.cache_clear()
    except Exception:
        pass
    paddlex_deps.is_extra_available = _patched_is_extra_available
    try:
        paddlex_deps.is_extra_available.cache_clear()
    except Exception:
        pass
...
class LocalPaddleOCRReader:
    ...
    def ensure_loaded(self):
        with self._load_lock:
            ...
            self.loading = True
            try:
                _debug_log("ensure_loaded: begin")
                ...
                with warnings.catch_warnings():
                    ...
                    try:
                        ...
                        _debug_log(f"ensure_loaded: primary init failed: {first_err!r}")
                        legacy_kwargs = dict(
                            device="cpu",
                            enable_hpi=False,
                            enable_mkldnn=False,
                ...
                self.ready = True
                self.error = ""
                _debug_log("ensure_loaded: success")
            except Exception as e:
                import traceback
                traceback.print_exc()
