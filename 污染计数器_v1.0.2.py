...
DEFAULT_CONFIG = {
    ...
    "resolution_presets": {
        "1920x1080": {
            "middle_region": {"left": 793, "top": 146, "width": 69, "height": 44},
            "header_region": {"left": 1594, "top": 22, "width": 279, "height": 112},
            "name_in_header": {"left": 74, "top": 26, "width": 153, "height": 36}
        },
        "2560x1440": {
            "middle_region": {"left": 1057, "top": 195, "width": 92, "height": 59},
            "header_region": {"left": 2125, "top": 30, "width": 372, "height": 150},
    ...
    "release_page_url": "https://github.com/YUZE04/Roco-pollution-counter/releases",
...
class LocalEasyOCRReader:
    """与母版一致：EasyOCR 单次加载；readtext 直接接收预处理后的 ndarray。"""
...
class App:
    def __init__(self):
        ...
        self.compact_hint_var = tk.StringVar(
            value=f"提示: {_sk0} 启/关  {_pk0} 暂停  {_lk0} 锁定/移动  {_ak0} 加污染  {_uk0} 减污染"
        )
    ...
    def set_window_lock(self, locked: bool):
        ...
        if self.window_locked:
            self.status_var.set("已锁定：全窗口穿透")
            self._set_cursor_hidden(True)
        else:
            self.status_var.set("可移动：窗口可点击与拖拽")
    ...
    def load_config(self):
        if CONFIG_FILE.exists():
            try:
                ...
                cfg.setdefault("resolution_presets", dict(DEFAULT_CONFIG["resolution_presets"]))
                cfg["resolution_presets"]["2560x1440"] = dict(DEFAULT_CONFIG["resolution_presets"]["2560x1440"])
                cfg["resolution_presets"]["1920x1080"] = dict(DEFAULT_CONFIG["resolution_presets"]["1920x1080"])
    ...
    def _build_settings_form(self, frame):
        ...
        tk.Label(row_lock, text="锁定/移动 (= 穿透)", fg="white", bg="#1a1023", font=UI_FONT).pack(side="left", padx=(0, 4))
        ...
        res_menu = tk.OptionMenu(row6, self.resolution_var, "1920x1080", "2560x1440", "2560x1600_150缩放")
        res_menu.config(font=UI_FONT, relief=tk.FLAT, highlightthickness=1, highlightbackground="#4a3d5c")
        try:
            res_menu["menu"].config(font=UI_FONT)
        except Exception:
            pass
        res_menu.pack(side="left")
        self._pill_button(row6, text="应用分辨率", command=self.on_apply_resolution).pack(side="left", padx=6)
    ...
    def build_compact_ui(self):
        ...
        self.runtime_status_label.pack(side="left", padx=10, pady=7)
        self._pill_button(status_strip, text="详情", command=self.show_species_stats, compact=True).pack(side="right", padx=(0, 8), pady=4)
        self._pill_button(status_strip, text="设置", command=self.open_settings_dialog, compact=True).pack(side="right", padx=(0, 8), pady=4)
        ...
        hotkey_text = f"启/关 {hk['start']}   暂停 {hk['pause']}   锁/移 {hk.get('lock', '=')}   +污 {hk['add']}   -污 {hk['sub']}"
    ...
    def _apply_running_window_mode(self):
        ...
        enabled = ga and bool(self.running) and bool(self.window_locked) and bool(self.in_compact_mode)
        try:
    ...
    def refresh_runtime_status(self):
        if self.running and self.paused:
            ...
            base = "未启动"
        lock_text = "已锁定" if self.window_locked else "可移动"
    ...
    def refresh_hotkey_tip(self):
        ...
        self.tip_detail_var.set(
            f'启/关:{hk["start"]}  暂停:{hk["pause"]}  锁定/移动:{hk.get("lock", "-")}  +污:{hk["add"]}  -污:{hk["sub"]}'
        )
        self._update_lock_visual_state()
    ...
    def stop_monitor(self):
        ...
        self.set_compact_hint(f"提示: {sk} 启动/停止，{pk} 暂停，{lk} 锁定/移动，{ak} 加污染，{uk} 减污染")
        self._set_window_no_activate(False)
