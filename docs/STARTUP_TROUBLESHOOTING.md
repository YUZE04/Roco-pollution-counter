...
2. **进入应用目录**
   ```bash
   cd "C:\Users\你的用户名\Downloads\污染计数器"
   ```
...
**解决：**
- 重新解压完整发布包，确保 `paddleocr_models` 目录完整
- 或手动把本地模型目录放回仓库 / exe 同级目录
...
## 收集诊断信息（提交到 GitHub Issue）

如果问题无法自己解决，建议新建 Issue 并提供：
...
3. **系统信息**
   - 运行 `py tests/diagnose_startup.py` 的输出
...
```bash
py -c "from pathlib import Path; from app.backend.paths import RUNTIME_DIR; print(Path(RUNTIME_DIR) / 'startup_error.log')"
```
